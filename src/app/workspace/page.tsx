    ...
    <div className="mobile-screen-h w-full flex flex-col bg-slate-100 dark:bg-zinc-950 overflow-hidden select-none">
      ...
      <header className="h-14 bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-2 sm:px-4 shrink-0 z-20">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          {/* Universal Panel Toggle Button (Desktop & Mobile) */}
          <button
            onClick={() => {
              if (isMobile) {
                setMobilePanelOpen(!mobilePanelOpen);
              } else {
                setRightPanel(rightPanel === "none" ? "properties" : "none");
              }
            }}
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border transition-all text-xs font-semibold ${
              (isMobile ? mobilePanelOpen : rightPanel !== "none")
                ? 'bg-primary text-white border-primary shadow-sm'
                : 'bg-slate-100 dark:bg-zinc-800 border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-slate-300 hover:bg-slate-200'
            }`}
            title="Toggle Sidebar Panel"
          >
            <Sparkles className="w-4 h-4" />
            <span className="hidden sm:inline">
              {(isMobile ? mobilePanelOpen : rightPanel !== "none") ? "Hide Sidebar" : "Show Sidebar"}
            </span>
          </button>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-hidden" style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row' }}>
        ...
        <main className="flex-1 relative bg-[#e2e8f0] dark:bg-[#09090b] overflow-hidden">
          ...
          {/* Floating Show Sidebar Panel Button (appears when sidebar is hidden) */}
          {((isMobile && !mobilePanelOpen) || (!isMobile && rightPanel === "none")) && (
            <button
              onClick={() => {
                if (isMobile) {
              ...
              className="absolute top-4 right-4 z-20 flex items-center gap-1.5 px-3 py-2 rounded-xl shadow-xl border text-xs font-bold transition-all hover:scale-105 bg-white/95 dark:bg-zinc-900/95 border-slate-200 dark:border-zinc-700 text-primary hover:bg-slate-50 dark:hover:bg-zinc-800"
            >
              <Sparkles className="w-4 h-4" />
        ...
        >
          ...
          <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200 dark:border-zinc-800 bg-slate-100 dark:bg-zinc-800/80 shrink-0">
